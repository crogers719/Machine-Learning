...
public class TicTacToe_Smart {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException, ClassNotFoundException {
			...
			String response;
			char answer;
			boolean player1Smart=false;
			boolean player2Smart=false;
			//List of all states. Each player has own stateArray.
			System.out.println("Woule you like to import a player 1 for a file ('Y' or 'N')");
			response=kbd.next();
			response=response.toUpperCase();
			answer=response.charAt(0);
			if (answer =='Y'){
				System.out.println("Enter the player1 filename");
				String fileName=kbd.next();
		   		
				System.out.println("\nReading PLAYER1 info from"+fileName+"...");	
				ObjectInputStream inputStream = new ObjectInputStream(new FileInputStream(fileName));
				player1StateArray = (ArrayList<String>)inputStream.readObject();
				player1StrategyGroup = (ArrayList<String>)inputStream.readObject();
				player1Smart=true;
			}
			else{
			...
			//List of all states. Each player has own stateArray.
			System.out.println("Would you like to import a player 2 for a file ('Y' or 'N')");
			response=kbd.next();
			response=response.toUpperCase();
			answer=response.charAt(0);
			if (answer =='Y'){
				System.out.println("Enter the player2 filename");
				String fileName=kbd.next();
		   		
				System.out.println("\nReading PLAYER2 info from"+fileName+"...");	
				ObjectInputStream inputStream = new ObjectInputStream(new FileInputStream(fileName));
				player2StateArray = (ArrayList<String>)inputStream.readObject();
				player2StrategyGroup = (ArrayList<String>)inputStream.readObject();
				player2Smart=true;
			}
			else{
				...
				if (player1Smart){
					...
					System.out.printf("\nPLAYER 1 SUCCESS RATE: %.2f%% " , (rate*100));
				}
				if (player2Smart){
					double rate=(double) player2Wins/NUM_OF_GAMES;
					System.out.printf("\nPLAYER 2 SUCCESS RATE: %.2f%% " , (rate*100));
				}
